...
## Acceptance Criteria
- [x] **Selection Logic**: The game prompts the player to pick a leader once at the start of the run.
- [x] **State Management**: The global game state correctly stores the `leaderHeroId`.
- [x] **Perk Application**: The specific modifier for the chosen leader is active (e.g., credit bonuses are calculated, scout reveals happen).
- [x] **Visual Identity**: The chosen leader is clearly distinguished in all game phases by a "Leader" icon or badge.
- [x] **One-Time Only**: The leader selection is locked and cannot be re-assigned during the run.

## Implementation Notes
- Compact UI implemented for 585px viewports (No Scroll Policy).
- Card JSX corruption repaired.
- 13 unique perks integrated into core loops.
